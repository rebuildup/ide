...
suite.test('Track tools', (ctx: TestContext) => {
  ...
  controller.trackTool('Read', { file_path: '/src/auth.ts' }, { content: 'export function login() {}' });
...
suite.test('Create snapshot', async (ctx: TestContext) => {
  ...
  assertEqual(session.snapshots[0].commitHash, snapshot.commitHash, 'Snapshot should be in session');
});
...
suite.test('Restore from snapshot', async (ctx: TestContext) => {
  ...
  const healthBeforeRestore = controller.getHealthScore();
...
suite.test('Compact preserves important events', async (ctx: TestContext) => {
  ...
  const errorCountBefore = beforeCompact.events.filter(e => e.type === 'error').length;
  const snapshotEventCountBefore = beforeCompact.events.filter(e => e.type === 'snapshot').length;
  ...
  const errorCountAfter = afterCompact.events.filter(e => e.type === 'error').length;
  const snapshotEventCountAfter = afterCompact.events.filter(e => e.type === 'snapshot').length;
  ...
  assert(totalEventsAfter < totalEventsBefore, `Total events should be reduced (before: ${totalEventsBefore}, after: ${totalEventsAfter})`);

  // A compact event should be added
  assert(compactEventCountAfter > compactEventCountBefore, `Compact event should be added (before: ${compactEventCountBefore}, after: ${compactEventCountAfter})`);
});
 ...
 */
suite.test('SessionAnalyzer drift threshold validation', (ctx: TestContext) => {
  const analyzer = new SessionAnalyzer(0.5);
...
async function runAllTests(): Promise<void> {
  ...
  await suite.test('SnapshotManager.findSnapshotsByHealth', (ctx) => {
    ...
    session.snapshots.push(
      { commitHash: 'abc123', timestamp: new Date().toISOString(), healthScore: 0.5, description: 'Low' },
      { commitHash: 'def456', timestamp: new Date().toISOString(), healthScore: 0.8, description: 'Medium' },
      { commitHash: 'ghi789', timestamp: new Date().toISOString(), healthScore: 0.95, description: 'High' }
    );
    ctx.store.update(ctx.sessionId, { snapshots: session.snapshots });
  ...
  await suite.test('SnapshotManager.deleteSnapshot', (ctx) => {
    ...
    session.snapshots.push(
      { commitHash: 'abc123', timestamp: new Date().toISOString(), healthScore: 0.8, description: 'Keep' },
      { commitHash: 'def456', timestamp: new Date().toISOString(), healthScore: 0.9, description: 'Delete' }
    );
    ctx.store.update(ctx.sessionId, { snapshots: session.snapshots });
  ...
  // SessionAnalyzer tests
  await suite.test('SessionAnalyzer drift threshold validation', (ctx) => {
    const analyzer = new SessionAnalyzer(0.5);
